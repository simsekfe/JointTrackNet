...
class DanceTrack(GenericDataset):
  num_classes = 1
  num_categories = 1
  ...
  def __init__(self, opt, split):
    ...
    print('Using DanceTrack {}'.format(split))

    data_dir = os.path.join(opt.data_dir, 'dancetrack')
    ...
    super(DanceTrack, self).__init__(opt, split, ann_path, img_dir)

    self.num_samples = len(self.images)
    print('Loaded DanceTrack {} {} samples'.format(split, self.num_samples))
  ...
  def save_results(self, results, save_dir):
    results_dir = os.path.join(save_dir, 'results_dancetrack')
